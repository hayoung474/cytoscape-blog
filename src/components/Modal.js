...
function Modal({
  ...
  useEffect(() => {
    const escKeyDownClose = (e) => { // esc 버튼을 클릭하였을 경우
      if (e.keyCode === 27) {
    ...
    const handleClickOutside = (e) => { 
      ...
      if (!modalEl.current || !wrapperEl.current) { //modalEl과 wrapperEl를 제외한 것을 클릭하였을 경우
  ...
  const changeLabel = () => {
    ...
    newGraph.nodes.forEach((item) => {
      if (item.data.label === currentNodeLabel) { // 만약에 현재 선택한 라벨과 동일한 라벨을 가진 노드를 찾았다면
  ...
  const addToEdgeNode = () => {
    ...
    newGraph.edges.forEach((item, index) => {
      if (item.data.id === connectedNodes[2]) { //connectedNodes[2] 는 현재 선택한 edge의 id값임.
  ...
  const deleteNodeAll = () => {
    ...
    deleteNodeList.forEach((node) => { 
      ...
      newGraph.edges.forEach((item, index) => {
        if (item.data.id.includes(node)) { // edge의 id에 삭제대상 노드의 id가 포함되어있다면(연관간선여부 판단)
