    ...
    return (
      <div>
        <ToggleButton
          ...
          title="메뉴 열기 및 닫기"
        ></ToggleButton>
        <SubButton visible={toggle} one onClick={openInfoModal} title="블로그 소개">
...
const ToggleButton = styled.button`
  ...
  &:before {
    transform: ${props => (props.visible === true ? 'rotate(45deg)' : 'rotate(180deg)')};
    transition: all 0.25s;
  }
  &:after {
    transform: ${props => (props.visible === true ? 'rotate(-45deg)' : 'rotate(-90deg)')};
    transition: all 0.25s;
  }
`;
